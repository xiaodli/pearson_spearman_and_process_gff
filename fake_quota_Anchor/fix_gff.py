...
def read_gff(gff_file):     # "/home/xiaodong/Desktop/GCF_902167145.1_Zm-B73-REFERENCE-NAM-5.0_genomic.gff"
        ...
        for line in f:
            ...
            if m is not None:
                ...
                if m is not None:
                    ...
                    trans_name = m.group(8)
                    gene_name = m.group(9)
                    trans_gene_map[trans_name] = gene_name
    ...
    for ch in chr_trans_cds_dict:
        for tran in chr_trans_cds_dict[ch]:
            ...
            ge_na = trans_gene_map[tran]
            transcript = chr_trans_cds_dict[ch][tran]
            chr_gene_trans_cds_dict[ch][ge_na] = Gene(ge_na, transcript.strand, ch)
            chr_gene_trans_cds_dict[ch][ge_na].add_trans(transcript)
            chr_gene_trans_cds_dict[ch][ge_na].update_order()   # dict includes attr of ordered transcript, chr_name, start, end, strand.
            gene_array = np.append(gene_array, [[chr_gene_trans_cds_dict[ch][ge_na]]], axis=0)
