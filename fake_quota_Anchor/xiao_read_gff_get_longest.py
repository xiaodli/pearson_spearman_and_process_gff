...
class Transcript:
    ...
    def __gt__(self, other):
        if self.distance > other.distance:
            return True
        elif self.start == other.start and self.start > other.start:
            return True
        else:
            return False
...
def read_gff(gff_file):
    ...
    with open(gff_file) as f:
        for line in f:
            ...
            if m is not None:
                ...
                if chr not in dict_gene_transcript_pep:
                    dict_gene_transcript_pep[chr] = dict()
                else:
                    if gene_name not in dict_gene_transcript_pep[chr]:
                        dict_gene_transcript_pep[chr][gene_name] = np.empty([0, 1], dtype=Transcript)
                    transcript = Transcript(start, end, trans_name, end-start)
                    dict_gene_transcript_pep[chr][gene_name] = np.append(dict_gene_transcript_pep[chr][gene_name], [[transcript]], axis=0)
        for ch in dict_gene_transcript_pep:
            ...
            for ge in dict_gene_transcript_pep[ch]:
                ...
                trans_list.append(dict_gene_transcript_pep[ch][ge][0][0].trans_name)
                # print(trans_list[i])
    ...
    # return None
# else:
#     # chr_trans_pep_map is chr : trans_name : pep_name.
#     m = re.search("^(\\S+)\t(\\S+)\tCDS\t(\\S+)\t(\\S+)\t(\\S+)\t(\\S+)\t(\\S)\tID=CDS:(.+?);Parent=transcript:(.+?);", line)
#     if m is not None:
#         chr = str(m.group(1))
#         pep_name = str(m.group(8))
#         trans_name = str(m.group(9))
#         if chr not in chr_trans_pep_map:
#             chr_trans_pep_map[chr] = dict()
#         if trans_name not in chr_trans_pep_map[chr]:
#             chr_trans_pep_map[chr][trans_name] = pep_name
        ...
        for line in f:
            ...
            if m is not None:
                ...
                if gene_name not in gene_to_chr:
                    gene_to_chr[gene_name] = chr
                if chr not in dict_chr_gene_gene_class:
                    dict_chr_gene_gene_class[chr] = np.empty([0, 1], dtype=Gene)
                gene = Gene(chr, start, end, strand, gene_name)
                dict_chr_gene_gene_class[chr] = np.append(dict_chr_gene_gene_class[chr], [[gene]], axis=0)
